...
/**
 * Taken from: https://codereview.stackexchange.com/questions/116686/simple-data-structures-for-representing-graphs-in-java
 *
 * This class defines the API for graph data structure. The actual nodes are
 ...
 * returns a boolean value indicating whether the structure of the graph
 * has changed.
...
public abstract class AbstractGraph {
     ...
     *          the same as that the added node was not present.
     */
     ...
     * @return {@code true} if the query node is in this graph.
     *          {@code false} otherwise
     */
    public abstract boolean hasNode(int nodeId);

    /**
     * If {@code nodeId} is present in this graph, removes all edges incident on {@code nodeId}.
     *
     * @param nodeId the node to clear.
     * @return {@code true} if the node {@code nodeId} had at least one
     * incident edge and, thus, the structure of the graph changed.
     */
     ...
     * possible cases are an undirected edge and directed edge.
     *
     * If some of the input nodes are not present in this graph,
     * it will be created siliently.
     *
    ...
    /**
     * Returns the weight of the code {@code (tailNodeId, headNodeId)}. If the
     * query edge does not exist, return {@link java.lang.Double#NaN}.
     *
     * @param tailNodeId the tail node of the query edge.
     * @param headNodeId the head node of the query edge.
